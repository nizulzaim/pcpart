        ...
        <div class="col-md-12">
            <data-table class="sortable" v-depth="3">
                <form @submit.prevent="nextFunction">
                    <cards-content>
                        <div class="font-subhead no-margin">Select Psu</div>
                        ...
                        <table>
                            ...
                            <tbody>
                                <tr v-for="(v, index) in products" :key="v._id" style="cursor: pointer">
                                    ...
                                    <td >{{v.formFactor}}</td>
                                    <td >{{v.memorySlot}}</td>
                                    <td >{{v.maximumSupportedMemory}} GB</td>
                                </tr>
                                <tr v-if="!$subReady">
...
<script>
    ...
    export default {
        ...
        meteor: {
            subscribe: {
                productsSearch() {
                    return ["Motherboard", this.searchTerm];
                },
            },
            products() {
                let prods = Product.find({type: "Motherboard"});
